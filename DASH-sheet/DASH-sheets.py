...
#requirements for reading from a google sheet
import pandas as pd
#list of the balance of our accounts
ls_mizan =[]
...
def seedash():
    ...
    html.Div(("حساب شرکت:  ", ls_mizan[0] ), style = {
...
#reading data from our googlesheet    
def datacloud():
    global ramzarz
    global ddown
    global sood
    ...
    zarar = df['ضرر'][0]
    trade = df['ترید'][0]
    #appending the balances of our accounts in a list
    i = 0
    while(i<4):
        ele = df['میزان'][i]
        ls_mizan.append(ele)
